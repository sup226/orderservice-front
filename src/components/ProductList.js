...
const ProductList = ({ pageTitle }) => {
  ...
  // 현재 로딩중이니? -> 백엔드로부터 상품 목록 요청을 보내서 데이터를 받아오는 중인가?
  ...
  useEffect(() => {
    ...
    // npm install lodash
    // 쓰로틀링: 짧은 시간동안 연속해서 발생한 이벤트들을 일정 시간으로 그룹화하여
    // 마지막 이벤트 핸들러만 호출하게 하는 기능.
    // 디바운싱: 짧은 시간동안 연속해서 발생한 이벤트를 호출하지 않다가
    // 마지막 이벤트로부터 일정 시간 이후에 한 번만 호출하게 하는 기능
    const throttledScroll = throttle(scrollPagination, 1000); // 단위 밀리초
    window.addEventListener('scroll', throttledScroll);

    // 클린업 함수: 다른 컴포넌트가 렌더링될 때 이벤트 해제.
    return () => window.removeEventListener('scroll', throttledScroll);
  }, []);

  // userEffect는 하나의 컴포넌트에서 여러 개 선언이 가능합니다.
  ...
  const scrollPagination = () => {
    // (브라우저 창의 높이 + 현재 페이지에서 스크롤 된 픽셀 값)이 (스크롤이 필요없는 페이지 전체 높이에서 100px 이내)에 도달했는가?
    const isBottom =
